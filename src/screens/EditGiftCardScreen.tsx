...
import { EditGiftCardScreenProps } from '../navigation/types';
...
export const EditGiftCardScreen: React.FC<EditGiftCardScreenProps> = ({
  navigation,
  route,
}) => {
  const { giftCard } = route.params;
  const { scheduleNotificationsForCard, cancelNotificationsForCard } = useNotifications();
  ...
  const [formData, setFormData] = useState<GiftCardFormData>({
    ...
    expirationDate: giftCard.expirationDate ? format(giftCard.expirationDate, 'yyyy-MM-dd') : '',
    purchaseDate: giftCard.purchaseDate ? format(giftCard.purchaseDate, 'yyyy-MM-dd') : '',
  ...
  const handleSave = async () => {
    ...
    setLoading(true);
    try {
      await DatabaseService.updateGiftCard(giftCard.id, formData);
      
      // Cancel existing notifications and schedule new ones if expiration date changed
      await cancelNotificationsForCard(giftCard.id);
      if (formData.expirationDate) {
        const updatedCard = await DatabaseService.getGiftCardById(giftCard.id);
        if (updatedCard) {
          await scheduleNotificationsForCard(updatedCard);
        }
      }

      Alert.alert(
        'Success',
        'Gift card has been updated successfully!',
        [
          {
            text: 'OK',
    ...
    >
      <ScrollView style={styles.scrollView} showsVerticalScrollIndicator={false}>
        <Card style={styles.formCard}>
          <Card.Content>
            <Title style={styles.formTitle}>Edit Gift Card</Title>
